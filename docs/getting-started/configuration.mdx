...
Use any MySQL client to apply the backup to the database, clear the database first by running the following command:
```sql
$  mysql -p -u <username> -h <host> -P <port> <database_name>
...
```sql
$ mysql -p -u <username> -h <host> -P <port> <database_name> < backup_mysql_YYYYMMDDHHMMSS.sql
```
